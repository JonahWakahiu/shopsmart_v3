<x-app-layout>
        ...
        <nav class="text-sm font-medium text-slate-600 dark:text-slate-300 mb-2" aria-label="breadcrumb">
            <ol class="flex flex-wrap items-center gap-1">
                ...
                <li class="flex items-center gap-1">
                    <a href="#" class="hover:text-slate-900 dark:hover:text-white">Customers</a>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden="true"
                        stroke-width="2" stroke="currentColor" class="size-4">
                        <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
                    </svg>
    ...
    <div x-data="customers" class="rounded bg-slate-50 shadow">
            ...
            <table class="w-full text-left text-sm">
                <thead class="border-y border-slate-200 text-slate-800">
                    <tr>
                        ...
                        <th scope="col" class="p-4 whitespace-nowrap">Total Spent</th>
                        <th scope="col" class="p-4">Country</th>
                        <th scope="col" class="p-4 whitespace-nowrap">Last Order</th>
                        ...
                        <tr>
                            <td class="p-4">
                                <label :for="customer.id"
                                    ...
                                    <x-forms.checkbox ::id="customer.id" ::checked="checkAll" />
                                </label>
                            </td>
                            <td class="p-4">
                            ...
                            <td class="p-4" x-text="customer.orders_count"></td>
                            <td class="p-4 whitespace-nowrap" x-text="customer.orders_sum_total"></td>
                            <td class="p-4 whitespace-nowrap" x-text="customer.country"></td>
                            <td class="p-4 whitespace-nowrap" x-text="customer.last_order"></td>
                            <td class="p-4">
                                <div class="flex items-center gap-2">
                                    {{-- show customer --}}
                                    <a :href="'{{ route('customers.show', ':id') }}'.replace(':id', customer.id)"
                                    ...
                                    {{-- edit customer --}}
                                    <a :href="'{{ route('customers.edit', ':id') }}'.replace(':id', customer.id)"
                                    ...
                                    {{-- delete customer --}}
                                    <button @click="deleteCustomer(customer)" type="button"
        ...
        <div class="flex items-center gap-2 p-4 border-t border-slate-300">
            <x-forms.input-label for="rowsPerPage">Rows per page
                <x-forms.select-input x-model="rowsPerPage" id="rowPerPage" @change="customersList">
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('customers', () => ({
                    ...
                    init() {
                        this.customers = @json($customers).data;
                        this.from = @json($customers).from;
                        this.to = @json($customers).to;
                        this.total = @json($customers).total;
                        this.currentPage = @json($customers).current_page;
                        this.totalPages = @json($customers).last_page;
