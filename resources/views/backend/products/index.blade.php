<x-app-layout>
        ...
        <nav class="text-sm font-medium text-slate-600 dark:text-slate-300 mb-2" aria-label="breadcrumb">
            <ol class="flex flex-wrap items-center gap-1">
                ...
                <li class="flex items-center gap-1">
                    <a href="#" class="hover:text-slate-900 dark:hover:text-white">Orders</a>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden="true"
                        stroke-width="2" stroke="currentColor" class="size-4">
                        <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
                    </svg>
    ...
    <div x-data="orders" class="rounded bg-slate-50 shadow">
                        ...
                        <tr>
                            <td class="p-4">
                                <label :for="order.id"
                                    ...
                                    <x-forms.checkbox ::id="order.id" ::checked="checkAll" />
                                </label>
                            </td>
                            <td class="p-4" x-text="order.order_id"></td>
                            <td class="p-4" x-text="order.total"></td>
                            <td class="p-4">
                                <div class="flex w-max items-center gap-2">
                                    <img class="size-8 rounded-full object-cover" :src="order.user.avatar"
                                        alt="customer avatar" />
                                    <div class="flex flex-col">
                                        <span class="text-slate-900" x-text="order.user.name">Alice Brown</span>
                                    </div>
                                </div>
                            </td>
                            <td class="p-4">
                                <div class="text-xs px-2 py-0.5 rounded flex items-center gap-2 w-fit capitalize border"
                                    :class="{
                                        'bg-orange-100 text-orange-500 border-orange-500': order.payment.status ==
                                            'unpaid',
                                        'bg-green-100 text-green-500 border-green-500': order.payment.status ==
                                            'completed',
                                        'bg-red-100 text-red-500 border-red-500': order.payment.status == 'failed',
                                        'bg-indigo-100 text-indigo-500 border-indigo-500': order.payment.status ==
                                            'refunded',
                                    }">
                                    <span x-text="order.payment.status">
                                    </span>
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="size-3"
                                        fill="currentColor">
                                        <path x-cloak x-show="order.payment.status == 'unpaid'"
                                            d="M464 256A208 208 0 1 1 48 256a208 208 0 1 1 416 0zM0 256a256 256 0 1 0 512 0A256 256 0 1 0 0 256zM232 120l0 136c0 8 4 15.5 10.7 20l96 64c11 7.4 25.9 4.4 33.3-6.7s4.4-25.9-6.7-33.3L280 243.2 280 120c0-13.3-10.7-24-24-24s-24 10.7-24 24z" />
                                        <path x-cloak x-show="order.payment.status == 'completed'"
                                            d="M438.6 105.4c12.5 12.5 12.5 32.8 0 45.3l-256 256c-12.5 12.5-32.8 12.5-45.3 0l-128-128c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0L160 338.7 393.4 105.4c12.5-12.5 32.8-12.5 45.3 0z" />
                                        <path x-cloak x-show="order.payment.status == 'failed'"
                            ...
                            <td class="p-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    {{-- edit order --}}
                                    <a :href="'{{ route('orders.edit', ':id') }}'.replace(':id', order.id)"
                                    ...
                                    {{-- delete order --}}
                                    <button @click="deleteOrder(order)" type="button"
        ...
        <div class="flex items-center gap-2 p-4 border-t border-slate-300">
            <x-forms.input-label for="rowsPerPage">Rows per page
                <x-forms.select-input x-model="rowsPerPage" id="rowPerPage" @change="ordersList">
            ...
            document.addEventListener('alpine:init', () => {
                Alpine.data('orders', () => ({
                        ...
                        try {
                            if (confirm('Are you sure?')) {
                                const response = await axios.delete(
                                    `{{ route('orders.destroy', ':id') }}`.replace(':id',
                                        order.id));
                    ...
                    init() {
                        ...
                        this.from = @json($orders).from;
                        this.to = @json($orders).to;
                        this.total = @json($orders).total;
                        this.currentPage = @json($orders).current_page;
                        this.totalPages = @json($orders).last_page;
